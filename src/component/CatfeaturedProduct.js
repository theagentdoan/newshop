...
class CatFeaturedProduct extends Component {
  constructor(props) {
    super(props);
    this.state = {
    ...
    this.productPerPage = 4;//Số sản phẩm hiện thị trong 1 trang
  ...
  fetch(url)
    ...
    .then((data) => {
      ...
      let currentProducts = this.getCurrentProducts(1);
      //Tính tổng số trang
      this.numberPages = Math.ceil(data.length / this.productPerPage);
      //Cập nhật state
      this.setState({ 
        currentProducts: currentProducts, currentPage:1,
      });
  ...
  componentDidMount() {
    // if (this.props.match.params.category != this.category)
    //   {return <NoMatch/>};
    //   return
      this.loadAPIdata(this.props.match.params.category);
      
  }
  shouldComponentUpdate(nextProps, nextState)
  {
    console.log(nextProps.match.params.category, this.props.match.params.category);
...
export default withRouter(CatFeaturedProduct);
